...
int				get_human_readable_nblen(long int nb, long int divider)
{
	...
	len = 0;
	//ft_bprintf(0, "Nb after divide = %f\n", dnb);
	if (dnb < 10)
		dnb = ft_centiceil(dnb);
	else
		dnb = ft_ceil(dnb);
	nb = (long int)dnb;
	//ft_bprintf(0, "Nb after rounding = %f\n", dnb);
	while (nb > 0)
	{
		nb /= 10;
		len++;
